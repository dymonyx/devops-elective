...
- **Условие: Для каждого контейнера при его запуске должны создаваться новые namespaces (PID, Mount, UTS)**
...
- **Условие: Для каждого контейнера с идентификатором <id> должна создаваться директория /var/lib/{имя-утилиты}/{id}**
...
- **Условие: В качестве rootfs использовать Alpine, но `chroot` делать на overlayfs**
  - `mount_overlay()` — монтирует overlayfs с lowerdir, upperdir, workdir на merged
  - `enter_rootfs()` — выполняет chroot в merged директорию

- **Условие: Запускаемая команда становится PID=1 и утилита ждет её завершения (foreground)**
...
- **Условие: Настроить cgroups для ограничения ресурсов контейнера (CPU, память, IO)**
  - `create_ram_cgroup()` — создает cgroup директорию и устанавливает лимит памяти `memory.max = 100M`
  - `clean_up_cgroup()` — удаляет cgroup директорию после завершения контейнера

- **Условие: Внутри контейнера монтировать /proc для корректной работы утилит типа ps**
